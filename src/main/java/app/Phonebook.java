...
public class Phonebook {
	...
	public boolean addNumber(Person person, String number) {
		...
		query = "INSERT INTO `phone` ( `owner`, `number`) VALUES ('" + person.getId() + "', '" + number + "')";
		...
		if (affected_rows > 0) {
			...
			person.getPhones().put(this.db.getLastInsertId().toString(), number);
	...
	public boolean updateNumber(Person person, String phoneID, String number) {
		...
		query = "UPDATE `phone` SET `number` = '" + number + "' WHERE `id` = " + phoneID;

		Integer affected_rows = this.db.changeDBData(query);
		System.out.println(affected_rows);
		if (affected_rows > 0) {
			person.editNumber(person, phoneID, number);
	...
	public boolean deleteNumber(Person person, String phoneID) {
		if ((phoneID != null) && (!phoneID.equals("null"))) {
			...
			if (affected_rows > 0) {
				// Удаляем запись о номере из общего списка.
				person.deleteNumber(person, phoneID);
				return true;
			} else {
				return false;
			}
